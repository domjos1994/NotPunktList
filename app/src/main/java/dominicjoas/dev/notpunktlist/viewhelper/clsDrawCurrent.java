...
public class clsDrawCurrent extends SurfaceView implements Runnable {
    Thread thread = null;
    ...
    public clsDrawCurrent(Context context) {
        super(context);
    ...
    private void drawLine(Canvas cvs, boolean touch) {
        cvs.drawColor(Color.WHITE, PorterDuff.Mode.CLEAR);
    ...
    public void calcDiagram(boolean touch) {
        if(surfaceHolder.getSurface().isValid()) {
            Canvas cvs = surfaceHolder.lockCanvas();
            this.maxWidth = cvs.getWidth();
            this.maxHeight = cvs.getHeight();
